...
public static class HidHideHelper
{
    ...
    public static void HideController(IHidHideService hidHideService, ControllerViewModelBase controller)
    {
        try
        {
            ...
            // Get the device instance ID using the controller's MAC address
            string? deviceInstanceId = GetDeviceInstanceIdByController(hidHideService, controller);
    ...
    /// <summary>
    /// Gets the device instance ID for a controller using its MAC address.
    /// </summary>
    /// <param name="hidHideService">The HidHide service to use for lookup</param>
    /// <param name="controller">The controller to find device instance ID for</param>
    /// <returns>The device instance ID if found, otherwise null</returns>
    private static string? GetDeviceInstanceIdByController(IHidHideService hidHideService, ControllerViewModelBase controller)
    {
        try
        {
            ...
            return hidHideService.FindDeviceInstanceIdByMacAddress(macAddress);
        }
        catch (Exception ex)
        {
            Logger.Error<TrayIconService>($"Failed to get device instance ID for controller '{controller.Name}': {ex.Message}");
            Logger.LogExceptionDetails<TrayIconService>(ex);
            return null;
        }
    }
