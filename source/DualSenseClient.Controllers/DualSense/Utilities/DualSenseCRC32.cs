...
namespace DualSenseClient.Controllers.DualSense.Utilities;

/// <summary>
/// CRC32 computation for DualSense Bluetooth reports using the seeded form (§7).
/// </summary>
/// <remarks>
/// The DualSense appends a 4-byte CRC32 to every Bluetooth output report. This
/// implementation uses the <b>seeded form</b>: standard CRC32 with polynomial
/// <c>0xEDB88320</c>, an initial value of <c>~seed</c>, and a final complement, but
/// with the table transformed so that the caller passes <c>seed</c> directly and
/// omits the complement. The seed <c>0xEADA2D49</c> equals <c>crc32([0xA2])</c> (the
/// HIDP output-report prefix). The controller rejects any Bluetooth report with
/// an invalid checksum.
/// </remarks>
public static class DualSenseCRC32
{
    ...
    /// Logger instance
    /// </summary>
    private static readonly DualSenseClientLogger _log = DualSenseClientLogger.For("CRC32DualSense");

    /// <summary>
    /// CRC32 lookup table pre-transformed for direct seeded-form computation.
    /// Each entry equals <c>std_crc32_table[i ^ 0xFF] ^ 0xFF000000</c>, which
    /// removes the need for the <c>~seed</c> initialization and final <c>~crc</c>
    /// complement that standard CRC32 requires.
    /// </summary>
    ...
    /// <summary>
    /// Computes the DualSense Bluetooth output report CRC32 over a region of data.
    /// Uses the seeded form with seed <c>0xEADA2D49</c> (HIDP prefix <c>0xA2</c>).
    /// </summary>
    /// <param name="data">Buffer containing the report data (without HIDP prefix).</param>
    /// <param name="offset">Starting index within <paramref name="data"/>.</param>
    /// <param name="length">Number of bytes to include in the checksum (typically 74 for BT output).</param>
    /// <returns>Computed CRC32 value.</returns>
    public static uint Compute(byte[] data, int offset, int length)
    {
        const uint HASH_SEED = 0xeada2d49;
        uint hashResult = HASH_SEED;

        _log.Trace($"Computing CRC32: offset={offset}, length={length}");
        ...
        _log.Trace($"CRC32 result: 0x{hashResult:X8}");
        return hashResult;
    }
}